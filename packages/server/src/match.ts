...
import type { GameEvent, MatchSummary } from '@worms-arena/shared';
import { Broadcaster } from './broadcast.js';
import { PRESET_AGENTS, type AgentProfile } from './ai/agent.js';
...
interface MatchSlot {
  ...
  agents: [AgentProfile, AgentProfile] | null;
  startedAt: number;
  /** Countdown seconds until next match in this slot (0 = playing) */
  countdown: number;
}
...
export function getWeaponStats(): Record<string, number> {
  return { ...weaponStats };
}
...
export class MatchManager {
  ...
  private agents: AgentProfile[] = PRESET_AGENTS;
  ...
  getNextCountdown(): number {
    let min = Infinity;
    for (const slot of this.slots) {
      if (slot.countdown > 0 && slot.countdown < min) min = slot.countdown;
    }
    return min === Infinity ? 0 : min;
  }

  /** Run an infinite match loop for one slot. */
  private async runSlotLoop(slot: MatchSlot, initialDelay: number): Promise<void> {
  ...
  private async runMatch(slot: MatchSlot): Promise<void> {
    ...
    while (!engine.isFinished && this.running) {
      ...
      if (!activeWorm) {
        ...
        if (!engine.isFinished) engine.advanceTurn();
        continue;
      }
    ...
    console.log(
      `[match-${slot.id}] ${matchId.slice(0, 8)} finished: ${
        engine.winnerId !== null ? agents[engine.winnerId].name + ' wins' : 'Draw'
      } (${engine.turnNumber} turns)`,
    );
  ...
  private pickAgents(): [AgentProfile, AgentProfile] {
