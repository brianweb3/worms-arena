...
import type { Worm, TurnAction, WeaponId, Wind, Inventory } from '@worms-arena/shared';
import {
  WEAPONS,
  WORM_RADIUS,
  MAP_WIDTH,
...
export function decideAction(
  ...
  const enemies = allWorms.filter(
    (w) => w.alive && w.teamId !== activeWorm.teamId,
  );
  ...
  const targets: Target[] = enemies.map((enemy) => {
  ...
  const shouldMoveTo = 
    moveChance > agent.aggression * 0.6 + agent.riskTolerance * 0.2 &&
    target.distance > 100 &&
    target.distance < 400;
  ...
  const shouldMove =
    moveChance > agent.aggression * 0.7 + agent.riskTolerance * 0.3 &&
    target.distance < 80 &&
    agent.riskTolerance < 0.4;

  if (shouldMove) {
    const dir = target.worm.x > activeWorm.x ? -1 : 1;
    return { type: 'move', direction: dir as -1 | 1 };
  }
  ...
  const { angle, power } = calculateAimBySimulation(
    activeWorm,
    target.worm,
    weaponId,
    wind,
...
function chooseWeapon(
  ...
  const availableWeapons: WeaponId[] = [];
  if (!inventory) {
    // Fallback: assume all weapons available
    availableWeapons.push('bazooka', 'grenade', 'shotgun');
  } else {
    if (inventory.weapons.bazooka > 0) availableWeapons.push('bazooka');
    if (inventory.weapons.grenade > 0) availableWeapons.push('grenade');
    if (inventory.weapons.shotgun > 0) availableWeapons.push('shotgun');
  }
...
function chooseTargetPosition(
  ...
  for (let i = 0; i < 10; i++) {
    ...
    // Check bounds
    if (candidateX < WORM_RADIUS || candidateX > MAP_WIDTH - WORM_RADIUS) continue;
    if (candidateY < WORM_RADIUS || candidateY > WATER_LEVEL - WORM_RADIUS) continue;

    // Check terrain
    const surfaceY = terrain.surfaceY(candidateX);
    if (Math.abs(candidateY - (surfaceY - WORM_RADIUS)) > 20) continue;

    // Score position
    const distToTarget = Math.sqrt(
      (candidateX - target.x) ** 2 + (candidateY - target.y) ** 2
    );
    
    let score = 0;
    // Prefer positions at good shooting distance
    if (distToTarget > 150 && distToTarget < 350) score += 50;
    // Prefer higher ground
    if (candidateY < target.y) score += 20;
    // Prefer cover (higher terrain nearby)
    score += 10;
    // Avoid too close
    if (distToTarget < 100) score -= 30;
 ...
 */
function calculateAimBySimulation(
  shooter: Worm,
  target: Worm,
  weaponId: WeaponId,
  wind: Wind,
  ...
  for (let ai = 0; ai < angleSteps; ai++) {
    ...
    for (let pi = 0; pi < powerSteps; pi++) {
      ...
      const distToTarget = Math.sqrt(
        (result.x - target.x) ** 2 + (result.y - target.y) ** 2,
      );
