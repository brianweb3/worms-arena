...
export class GameEngine {
  ...
  constructor(seed: number, matchId: string, agentIds: string[]) {
    ...
    for (let t = 0; t < TEAM_COUNT; t++) {
      const inventory: Inventory = {
        weapons: {
          bazooka: STARTING_WEAPONS.bazooka as number,
          grenade: STARTING_WEAPONS.grenade as number,
          shotgun: STARTING_WEAPONS.shotgun as number,
        },
  ...
  getActiveWorm(): Worm | null {
    const teamWorms = this.worms.filter(
      (w) => w.teamId === this.currentTeamIndex && w.alive,
    );
  ...
  checkWinCondition(): void {
    ...
    for (const w of this.worms) {
      if (w.alive) aliveTeams.add(w.teamId);
    }
  ...
  private executeShoot(
    worm: Worm,
    action: Extract<TurnAction, { type: 'shoot' }>,
    result: TurnResult,
  ): void {
    ...
    for (let p = 0; p < weapon.projectileCount; p++) {
      ...
      if (weapon.projectileCount > 1) {
        const spreadOffset =
          (p - (weapon.projectileCount - 1) / 2) * weapon.spreadAngle;
        angle += spreadOffset;
      }
      ...
      const projResult = simulateProjectile(
        startX,
        startY,
        vx,
        vy,
      ...
      if (projResult.explosion) {
        ...
        const { damagedWorms } = applyExplosion(
  ...
  private executeMove(
    ...
    for (let i = 0; i < Math.abs(WORM_MOVE_SPEED / 2); i++) {
      const nextX = x + step;
      if (nextX < WORM_RADIUS || nextX > MAP_WIDTH - WORM_RADIUS) break;
  ...
  private executeMoveTo(
    worm: Worm,
    action: Extract<TurnAction, { type: 'moveTo' }>,
    result: TurnResult,
  ): void {
    const startX = worm.x;
    const startY = worm.y;
    ...
    for (let i = 0; i < steps; i++) {
      ...
      if (nextX < WORM_RADIUS || nextX > MAP_WIDTH - WORM_RADIUS) break;
      if (nextY < WORM_RADIUS || nextY > WATER_LEVEL) break;
  ...
  private executeUseItem(
    worm: Worm,
    action: Extract<TurnAction, { type: 'useItem' }>,
    result: TurnResult,
  ): void {
    const team = this.teams[worm.teamId];
    if (!team) return;
  ...
  private spawnItem(x: number, y: number): void {
    const itemTypes: Array<{ type: ItemType; weaponId?: WeaponId }> = [
      { type: 'healthKit' },
      { type: 'shield' },
      { type: 'speedBoost' },
  ...
  private checkItemPickup(worm: Worm, result: TurnResult): void {
  ...
  private emptyTurnResult(action: TurnAction): TurnResult {
    return {
      turnNumber: this.turnNumber,
      activeWormId: -1,
      wind: this.wind,
